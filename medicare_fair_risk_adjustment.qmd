...
Strategic responses by insurers, the extent to which additional payments will impact beneficiaries' experiences, and the potential for such payments to mitigate disparities in healthcare access and outcomes were not included in our study. Other policy levers would need to be paired with algorithmic changes to the payment system to increase the likelihood that reform yields desired outcomes. Importantly, health care disparities are the result of many long-standing and interconnected social and structural forces. [@nationalacademiesofsciencesengineeringandmedicine2024] Algorithmic changes and market forces alone cannot eliminate disparities, but algorithms can be designed to support and contribute to a more equitable system. [@pmlr-v248-cusick24a]
...
```{r, echo = F}
reg_coefs <- fread("safe_figures/2018/disparities_targets_regression_coefficients.csv")
reg_coefs <- reg_coefs[HCC!="(Intercept)"]
reg_coefs <- reg_coefs[, HCC := fcase(
      HCC == "orig_dis", "First Eligible for Medicare Based on a Disabling Condition",
      HCC == "num_hcc", "Number of Diagnosed Payment-Eligible HCCs",
      HCC == "age_sexFemale_70-74", "Female 70-74",
      HCC == "age_sexFemale_70-74", "Female 70-74",
      HCC == "age_sexFemale_75-79", "Female 75-79",
      HCC == "age_sexFemale_80-84", "Female 80-84",
      HCC == "age_sexFemale_85-89", "Female 85-89",
      HCC == "age_sexFemale_90+", "Female 90+",
      HCC == "age_sexMale_65-69", "Male 65-69",
      HCC == "age_sexMale_70-74", "Male 70-74",
      HCC == "age_sexMale_75-79", "Male 75-79",
      HCC == "age_sexMale_80-84", "Male 80-84",
      HCC == "age_sexMale_85-89", "Male 85-89",
      HCC == "age_sexMale_90+", "Male 90+")]
reg_coefs <- rbind(reg_coefs, data.table(HCC = "Female 65-69", beta = 0, V1 = 3.5), fill = TRUE)
reg_coefs <- reg_coefs[, beta:=dollar(round(beta), big.mark = ",")]
reg_coefs <- reg_coefs[order(V1)]
setnames(reg_coefs, c("HCC", "beta"), c("Variable", "Coefficient"))
kable(reg_coefs[, .(Variable, Coefficient)])
```

###### eTable 2. Out-of-sample net compensation by algorithm, spending target, and racial/ethnic group for the 2018-2019 cohort. {#supplement_table_2}
...
###### eTable 3a. Sample size, characteristics, observed spending, and predicted spending, overall and by race/ethnicity for the 2017-2018 cohort. {#supplement_table_3}
...
###### eTable 4a. Observed spending and net compensation by algorithm, spending target, and racial/ethnic group for the 2017-2018 cohort. {#supplement_table_4}
...
###### eTable 5. Payment system fit across algorithms, spending targets, and cohorts. {#supplement_table_5}
